...
function notifyUser(userId, kind, title, body = '', fileId = null, link = '') {
  if (!userId) return;
  run(
 ...
 */
function notifyStaffForFile(file, kind, title, body = '', link = '') {
  const targets = new Set();
  if (file.assigned_broker_id) {
    targets.add(file.assigned_broker_id);
  } else if (getSetting('automation', {}).notify_all_staff_if_unassigned !== false) {
    for (const u of all("SELECT id FROM users WHERE role != 'client' AND status = 'active'")) targets.add(u.id);
  }
  for (const id of targets) notifyUser(id, kind, title, body, file.id, link);
}

/** Notify every portal user attached to a file (all applicants with access). */
function notifyClientsForFile(fileId, kind, title, body = '', link = '') {
  const users = all(
    'SELECT DISTINCT portal_user_id AS id FROM applicants WHERE file_id = ? AND portal_user_id IS NOT NULL',
    fileId
  );
  for (const u of users) notifyUser(u.id, kind, title, body, fileId, link);
}

function unreadCount(userId) {
  const row = get('SELECT COUNT(*) AS n FROM notifications WHERE user_id = ? AND read_at IS NULL', userId);
  return row ? row.n : 0;
}
